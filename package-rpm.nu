...
# RPM Packaging Script for Mechanix Browser
# Usage: nu package-rpm.nu browser <output-dir>
...
def collect-artifacts [app: record, build_dir: path, rpm_root: path, pkg_name: string] {
    ...
    let desktop_installed = if ($desktop_src | path exists) {
        ...
        if not ($desktop_dest | path exists) { mkdir $desktop_dest }
        cp $desktop_src ($desktop_dest | path join $desktop_file)
        true
    } else {
        false
    }
    ...
    let icon_installed = if ($icon_src | path exists) {
        ...
        if not ($icon_dest | path exists) { mkdir $icon_dest }
        cp $icon_src ($icon_dest | path join $icon_file)
        true
    } else {
        false
    }
...
def generate-spec [pkg_name: string, version: string, release: string, arch: string, app: record, pubspec: record, artifacts: record] {
    ...
    let base_lines = [
        ...
        "License: BSD"
        $"Packager: ($maintainer)"
        "URL: https://github.com/mineshp-mecha/mechanix-browser"
        $"BuildArch: ($arch)"
        "AutoReqProv: no"
    ]
